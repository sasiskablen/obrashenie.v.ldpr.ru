...
if (error) {
  ...
  async function initAdminPage() {
    ...
    tbody.innerHTML = rows.map(function (r) { return "<tr class='border-b'><td class='p-3'>" + formatDate(r.ticket.createdAt) + "</td><td class='p-3'>" + (r.user ? r.user.name : "-") + "</td><td class='p-3'>" + (r.user ? r.user.email : "-") + "</td><td class='p-3'>" + TOPICS[r.ticket.subject] + "</td><td class='p-3'><span class='status-pill " + STATUS_CLASSES[r.ticket.status] + "'>" + STATUS_LABELS[r.ticket.status] + "</span></td><td class='p-3'><button class='px-3 py-1 bg-blue-700 text-white rounded hover:bg-blue-800' data-open-ticket='" + r.ticket.id + "'>Открыть</button></td></tr>"; }).join("");
